...
11. **Not yet investigated:** the fall-through (fallback-chain) drill's behavior under Helicone is unconfirmed — `smoke-test.sh` prints the result rather than asserting on it, matching Bifrost's same provider-scoped-fallback caveat above.
